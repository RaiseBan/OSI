...
| Программа       | USER% | SYS% | WAIT% | Время выполнения | Переключения контекста |
|-----------------|-------|------|-------|------------------|------------------------|
| substring_search   | 97.5% | 2.5%| 0%    | 41.609s       | 5,729                 |
...
- **Параметры запуска**: `substring_search D:\large_text.txt R7 2`
- **Ожидаемое время выполнения**: ~2.5 секунды
- **Фактическое время выполнения**: 41.609 секунд
- **Оптимизации компилятора**: отключены по заданию
- **Код программы**: наивный алгоритм поиска подстроки

**Причины увеличения времени выполнения:**

1. **Неэффективный алгоритм поиска:**

   - **Наивный алгоритм:** Используется алгоритм со сложностью O(N*M), где N — длина текста, M — длина шаблона.
   - **Высокая вычислительная сложность:** При больших значениях N время выполнения существенно увеличивается.

2. **Операции с строками внутри цикла:**

   - **Создание подстроки:** В каждой итерации цикла вызывается `text.substr(i, pattern_len)`, что создает новую строку.
   - **Накладные расходы:** Создание и копирование строковых объектов в каждой итерации значительно замедляет выполнение.

3. **Сравнение строк:**

   - **Операция `==`:** Сравнение строк выполняется посимвольно.
   - **Отсутствие оптимизаций:** Без оптимизаций компилятор не может упростить или ускорить сравнение строк.

4. **Отключенные оптимизации компилятора:**

   - **Влияние на операции со строками:** Компилятор не оптимизирует работу с объектами строк, не использует SIMD-инструкции для ускорения операций.
   - **Последствия:** Увеличение времени выполнения операций создания и сравнения строк.

5. **Однопоточное выполнение и использование памяти:**

   - **Загрузка CPU:** Используется только одно ядро процессора.
   - **Большой рабочий набор:** Размер текста может быть значительным, что приводит к большим объемам используемой памяти.
   - **Кэш-промахи:** Частые обращения к оперативной памяти замедляют выполнение из-за медленного доступа по сравнению с кэш-памятью.

6. **чтение:**
   - Скорость чтения: ~25 MB/s, так как у нас файл 500 MB, то 20 секунд будет тратиться на чтение данных. Сам алгоритм отрабатывает за 21. а так как мы запускаем его 2 раза, следовательно по 10 секунд каждый.
**Вывод:**

Основной причиной увеличения времени выполнения программы `substring_search` является использование неэффективного наивного алгоритма поиска подстроки, усугубленное частым созданием и сравнением строк внутри цикла. Отключение оптимизаций компилятора значительно увеличивает накладные расходы на эти операции.
---

**Мини-заключение:**

Увеличение времени выполнения программ `binary_search` и `substring_search` объясняется сочетанием факторов: отключенные оптимизации компилятора, неэффективные алгоритмы и реализации, а также однопоточное выполнение. Для уменьшения времени выполнения необходимо оптимизировать код, использовать эффективные алгоритмы и, при возможности, задействовать многопоточность.
...
Для определения эффективного количества нагрузчиков, нагружающих все ядра процессора, выполним анализ полученных данных.

### Наблюдения по запуску нагрузчиков:
- **2 экземпляра**: Загрузка процессора находится на низком уровне, распределение нагрузки по ядрам неравномерное, полная загрузка не достигается.
- **4 экземпляра**: Процессорная загрузка возрастает, наблюдается более равномерное распределение нагрузки, хотя некоторые ядра еще не полностью задействованы.
- **8 экземпляров**: Все ядра процессора начинают активно работать, достигая более высоких значений загрузки.
- **12 экземпляров**: Процессор загружен практически на 100% на всех ядрах, достигается полная эффективность.

### Распределение времен и переключений контекста:
| Количество экземпляров | USER% | SYS% | WAIT% | Переключения контекста |
|-----------------------|-------|------|-------|------------------------|
| 2                     | Низкое | Низкое | ?    | Минимальное           |
| 4                     | Среднее | Умеренное | ?    | Увеличение             |
| 8                     | Высокое | Высокое | ?   | Значительное увеличение |
| 12                    | Максимальное | Высокое | ?    | Максимальное           |

#### Выводы:
- **Эффективное количество нагрузчиков** для полной загрузки системы составляет 12 экземпляров.
- При этом наблюдается максимальное значение USER%, близкое к 100%, и SYS%, что свидетельствует о полной загрузке CPU.
- **Количество переключений контекста** резко возрастает с увеличением числа экземпляров, что указывает на высокую интенсивность планирования задач операционной системой для поддержания работы всех экземпляров.
...
### Таблица 2: Нагрузка на CPU для многопоточной реализации (1, 2, 3 экземпляра)

   | Количество инстансов | USER% | SYS% | WAIT% | Контекстные переключения | Время выполнения |
   |--------------------|-------|------|-------|--------------------------|------------------|
   | x1           | 98.5% | 1.5%| ?%    | ?                  | 44.5 сек        |
   | x2           | 98.2% | 1.8%| ?%    | ?                  | 45.6 сек        |
   | x4           | 98.2% | 1.8%| ?%    | ?                   | 52.7 сек        |


**Вывод:** Использование потоков для параллельной обработки показало более эффективное распределение нагрузки на ядра CPU, что позволило сократить реальное время выполнения в 2 раза.
...
Для пятого пункта задания с оптимизацией:
...
1. **Binary Search** (`agressive_binary.exe 100000 1 250000000`):
   - **Время выполнения:** 0.05414 секунды.
...
2. **Substring Search** (`agressive_substring D:\large_text.txt R7 2`):
   - **Время выполнения:** 10.1633 секунды.
   - **Ранее** программа имела значительно более длительное время исполнения, что также указывает на существенное ускорение.
   - Графики показали, что данные теперь читаются сразу, загружая все 500 MB, а не частями по 25 MB/s. Это произошло из-за оптимизированного доступа к памяти и более эффективного использования кэширования данных, что ускоряет обработку больших файлов.

### Вывод:
Применение оптимизации `-O3` существенно сократило время выполнения обоих программ-нагрузчиков. Время выполнения **binary search** сократилось до долей секунды, а для **substring search** — в несколько раз, что подтверждает эффективность агрессивных оптимизаций для вычислительных задач с высокой нагрузкой на процессор.
