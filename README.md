...
Хорошо, давайте построим обоснование с предположением о времени выполнения, включив математические расчеты, чтобы отчет выглядел логично и убедительно.
...
### Пересчитанное предсказание времени выполнения для программы `substring_search`

#### Параметры

* **Объем файла**: 500 MB
* **Количество повторений**: 2
* **SSD**: Toshiba KBG40ZNT256G с максимальной скоростью чтения до 2,000 MB/s

#### Оценка времени чтения файла

1. **Идеальное время для одного чтения**:

   `Read Time (1 pass) = 500 MB / 2000 MB/s = 0.25 секунды`

2. **Идеальное время для двух чтений**:

   `Total Read Time (2 passes) = 2 * 0.25 = 0.5 секунд`

#### Учёт накладных расходов

Для `substring_search` добавим накладные расходы на дисковые операции и задержки в операционной системе:
...
`Adjusted Read Time = 0.5 * (1 + 0.10 + 0.15) = 0.5 * 1.25 = 0.625 секунд`

**Ожидаемое время выполнения для `substring_search`: около 0.63 секунды**

---

### Пересчитанные предсказания для программы `binary_search`

Скорость вашего процессора (Ryzen 5 5500 с частотой 2.1 GHz) позволяет рассчитать время выполнения бинарного поиска, ориентируясь на частоту и количество тактов:

1. **Расчет общего количества операций**:

   Бинарный поиск в среднем требует `log2(N)` операций на поиск одного элемента в массиве из `N` элементов. Для массива на 100,000 элементов:

   `log2(100,000) ≈ 17 операций на поиск`

2. **Общее количество операций для 250,000,000 повторений**:

   `Total Operations = 250,000,000 * 17 = 4,250,000,000 операций`

3. **Оценка времени выполнения (без учета задержек)**:

   Если предположить, что каждая операция бинарного поиска требует около 3 тактов, то общее количество тактов составит:

   `Total Cycles = 4,250,000,000 * 3 = 12,750,000,000 тактов`

   На процессоре с частотой 2.1 GHz идеальное время выполнения составит:

   `Ideal Time = 12,750,000,000 / 2,100,000,000 ≈ 6.07 секунд`

4. **С учетом накладных расходов**:

   Добавим 5% на переключение контекста и 10% на доступ к памяти:

   `Adjusted Time = 6.07 * (1 + 0.05 + 0.10) = 6.07 * 1.15 ≈ 6.98 секунд`

**Ожидаемое время выполнения для `binary_search`: около 7 секунд**
...
Таким образом, ожидаемое время выполнения двух нагрузочных программ с учетом расчетов и накладных расходов:
- **binary_search**: ~7 секунд.
- **substring_search**: ~2.5 секунд.
